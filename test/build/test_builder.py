...
class BuilderTest(unittest.TestCase):
    ...
    def test_builder_python_tests(self):
        ...
        t = saliweb.build.builder_python_tests(
            'dummytgt', ['foo.py', 'bar.py'], e)
        regex = r'.*python .*/run\-tests\.py --html_coverage=testcov ' \
                r'testser foo\.py bar\.py$'
        m = re.match(regex, e.exec_str)
        self.assertNotEqual(m, None, 'String %s does not match regex %s'
                            % (e.exec_str, regex))
    ...
    def test_builder_python_frontend_tests(self):
        ...
        e = DummyEnv(0)
        t = saliweb.build.builder_python_frontend_tests(
            'dummytgt', ['foo.py', 'bar.py'], e)
        self.assertEqual(t, None)
        ...
        regex = r'.*python .*/run\-tests\.py --frontend testser ' \
                r'foo\.py bar\.py$'
        m = re.match(regex, e.exec_str)
        self.assertNotEqual(m, None, 'String %s does not match regex %s'
                            % (e.exec_str, regex))
        ...
        t = saliweb.build.builder_python_frontend_tests(
            'dummytgt', ['foo.py', 'bar.py'], e)
        regex = r'.*python .*/run\-tests\.py --coverage --frontend testser ' \
                r'foo\.py bar\.py$'
        m = re.match(regex, e.exec_str)
        self.assertNotEqual(m, None, 'String %s does not match regex %s'
                            % (e.exec_str, regex))
        ...
        t = saliweb.build.builder_python_frontend_tests(
            'dummytgt', ['foo.py', 'bar.py'], e)
        regex = r'.*python .*/run\-tests\.py --html_coverage=testcov ' \
                r'--frontend testser foo\.py bar\.py$'
        m = re.match(regex, e.exec_str)
        self.assertNotEqual(m, None, 'String %s does not match regex %s'
                            % (e.exec_str, regex))
