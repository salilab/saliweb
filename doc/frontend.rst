...
To submit the job, call the :meth:`~saliwebfrontend.make_job`
...
by overriding the :meth:`~saliwebfrontend.get_results_page` method. Unlike the other
...
:meth:`~saliwebfrontend.allow_file_download` method can be overridden to control which files
the end user is allowed to download. Note also that if some files are not text
files, the :meth:`~saliwebfrontend.get_file_mime_type` method should also be overridden to
