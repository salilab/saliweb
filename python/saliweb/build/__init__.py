...
def _make_web_service(env, target, source):
    ...
    with open(target[0].path, 'w') as f:
        ...
        print("try:", file=f)
        print("    import %s.backend as backend" % modname, file=f)
        print("except ImportError:", file=f)
        print("    import %s as backend" % modname, file=f)
        print("def get_web_service(config):", file=f)
        print("    ws = backend.get_web_service(config)", file=f)
